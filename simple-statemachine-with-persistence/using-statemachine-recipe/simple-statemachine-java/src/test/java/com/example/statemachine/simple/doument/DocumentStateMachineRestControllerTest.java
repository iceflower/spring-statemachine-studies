...
class DocumentStateMachineRestControllerTest {
  ...
  @Nested
  @DisplayName("API Endpoint '/{documentId}/current-state' 는")
  class DescribeOf_currentState {

    @Nested
    @DisplayName("DB에 존재하지 않는 PathVariable 'documentId' 로 Get 요청이 주어지면")
    class ContextWhen_get_request1 {

      @Test
      @DisplayName("http code 422를 돌려준다")
      void it_returns_state_list() throws Exception {

        mockMvc.perform(get("/{documentId}/current-state", 5))
    ...
    @Nested
    @DisplayName("DB에 존재하는 PathVariable 'documentId' 로 Get 요청이 주어지면")
    class ContextWhen_get_request2 {
      ...
      void it_returns_state_list() throws Exception {
        ...
        mockMvc.perform(get("/{documentId}/current-state", 1))
          .andExpect(status().isOk())
  ...
  @Nested
  @DisplayName("API Endpoint '/{documentId}/send/{event}' 는")
  class DescribeOf_sendEvent {

    @Nested
    @DisplayName("DB에 존재하지 않는 PathVariable 'documentId' 가 주어지면")
    class ContextWhen_given_documentId_is_nonExist {

      private static Stream<Arguments> paramProvider() {
        return getDocumentEvents(Set.of());
      }

      @ParameterizedTest
      @MethodSource("paramProvider")
      @DisplayName("statemachine의 상태는 초기 상태인 'DRAFT' 에서 바뀌지 않고, HTTP code 는 422로 돌려준다")
      void it_returns_state_result(DocumentEvent event) throws Exception {
        ...
        mockMvc.perform(post("/{documentId}/send/{event}", 5, event))
          .andExpect(status().isUnprocessableEntity())
          .andDo(print());
    ...
    @Nested
    @DisplayName("DB에 존재하는 PathVariable 'documentId' 가 주어지고")
    class ContextWhen_given_documentId_is_exist {
      @Nested
      ...
      class ContextWhen_state_is_draft {
        ...
        class ContextWhen_get_request_with_DOCUMENT_PUBLISHED_BY_USER {
          ...
          void it_returns_state_list() throws Exception {
            ...
            mockMvc.perform(post("/{documentId}/send/{event}", 1, event))
              .andExpect(status().isOk())
